      ...
      <div className="space-y-4">
        ...
        <div className="bg-secondary/30 rounded-lg p-4 space-y-3">
          <p className="text-sm text-foreground leading-relaxed">
            {trainset.explanation?.finalReason ?? trainset.reason}
          </p>

          {trainset.explanation && (
            <div className="space-y-3">
              {trainset.explanation.blockers.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Blockers</p>
                  <div className="space-y-1">
                    {trainset.explanation.blockers.map((b) => (
                      <p key={b.code} className="text-xs text-ibl">- {b.message}</p>
                    ))}
                  </div>
                </div>
              )}

              {trainset.explanation.warnings.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Warnings</p>
                  <div className="space-y-1">
                    {trainset.explanation.warnings.map((w) => (
                      <p key={w.code} className="text-xs text-standby">- {w.message}</p>
                    ))}
                  </div>
                </div>
              )}

              {trainset.explanation.promoters.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Promoters</p>
                  <div className="space-y-1">
                    {trainset.explanation.promoters.map((p) => (
                      <p key={p.code} className="text-xs text-revenue">- {p.message}</p>
                    ))}
                  </div>
                </div>
              )}

              {trainset.explanation.overrides.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Overrides</p>
                  <div className="space-y-1">
                    {trainset.explanation.overrides.map((o) => (
                      <p key={o.code} className="text-xs text-muted-foreground">- {o.message}</p>
                    ))}
                  </div>
                </div>
              )}
